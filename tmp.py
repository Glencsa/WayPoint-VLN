    ...
    for item in data:
        ...
        for path in item.get('depth_images', []):
            if 'depth_images_r2r_train/' in path:
                ...
                new_path = f"dataset_waypoint/depth_images_r2r_train/{suffix}"
        ...
        for path in item.get('images', []):
            if 'rgb_images_points/' in path:
                ...
                new_path = f"dataset_waypoint/rgb_images_points/{suffix}"
...
if __name__ == "__main__":
    input_filename = "/root/autodl-tmp/RVLN/dataset_waypoint/rgb_images_r2r_train.json"
    output_filename = "/root/autodl-tmp/RVLN/dataset_waypoint/rgb_images_r2r_train_processed.json" # 保存为新文件以防覆盖
